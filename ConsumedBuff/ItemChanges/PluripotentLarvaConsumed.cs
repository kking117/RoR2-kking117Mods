    ...
    public class PluripotentLarvaConsumed
    {
        ...
        private static void UpdateText()
        {
            ...
            if (MainPlugin.VoidDio_CollapseChance > 0.0f)
            {
                ...
                desc += string.Format("<style=cIsDamage>{0}%</style> <style=cStack>(+{0}% per stack)</style> chance to <style=cIsDamage>collapse</style> an enemy for <style=cIsDamage>{1}%</style> ", MainPlugin.VoidDio_CollapseChance, MainPlugin.VoidDio_CollapseDamage * 100);
                if (MainPlugin.VoidDio_CollapseUseTotal)
                {
                    desc += "TOTAL damage.";
                }
                else
            ...
            if (MainPlugin.VoidDio_BlockCooldown <= 1.0f)
            {
                ...
                if (MainPlugin.VoidDio_BlockCooldown < 1.0f)
                {
                    desc += string.Format("<style=cStack>(-{0}% per stack)</style> ", (1.0f - MainPlugin.VoidDio_BlockCooldown) * 100);
                }
                desc += "seconds</style>.";
            }
            if (MainPlugin.VoidDio_Curse > 0f)
            {
                ...
                desc += string.Format("<style=cIsHealth>Reduces maximum health by {0}%</style> <style=cStack>(+{0}% per stack)</style>.", MainPlugin.VoidDio_Curse * 100f);
            }
            if (MainPlugin.VoidDio_Corrupt)
            {
                if (pickup.Length > 0)
        ...
        private static void CalculateStatsHook(CharacterBody sender, RecalculateStatsAPI.StatHookEventArgs args)
        {
            if (sender && sender.inventory)
            {
                ...
                if (itemCount > 0)
                {
                    if (MainPlugin.VoidDio_Curse > 0.0f)
                    {
                        args.baseCurseAdd += MainPlugin.VoidDio_Curse * itemCount;
                    }
                    if (MainPlugin.VoidDio_BlockCooldown <= 1.0f)
                    {
                ...
                if (attackerBody)
                {
                    ...
                    if (attackerMaster && attackerMaster.inventory)
                    {
                        ...
                        if (itemCount > 0 && Util.CheckRoll(dr.damageInfo.procCoefficient * itemCount * MainPlugin.VoidDio_CollapseChance, attackerMaster))
                        {
                            ...
                            if (MainPlugin.VoidDio_CollapseUseTotal)
                            {
                                damage = MainPlugin.VoidDio_CollapseDamage * dr.damageDealt / dotDef.damageCoefficient / attackerBody.damage;
                            }
                            else
                            {
                                damage = MainPlugin.VoidDio_CollapseDamage / dotDef.damageCoefficient;
                            }
